// 언어 : C++ , (성공/실패) : 1/0 , 메모리 : 5536 KB , 시간 : 72ms
#include <iostream>
#include <algorithm>
...
Tree queryRec(int left,int right,int node,int start,int end){
    ...
    int mid = (start + end) / 2;
    return merge(queryRec(left,right,node*2,start,mid), queryRec(left,right,node*2+1,mid+1,end));
}
